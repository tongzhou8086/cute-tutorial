...
@cute.kernel
def local_tile_copy_kernel(src: cute.Tensor, dst: cute.Tensor):
...
@cute.jit
def local_tile_copy(src: cute.Tensor, dst: cute.Tensor):
    rows, cols = src.shape
...
def parse_args():
    parser = argparse.ArgumentParser(description="CuTe DSL local_tile copy")
...
def main():
    ...
    compiled = cute.compile(local_tile_copy, src_cute, dst_cute)
    compiled(src_cute, dst_cute)
    torch.cuda.synchronize()

    torch.testing.assert_close(dst, src)
    max_abs_err = (dst - src).abs().max().item()
    print("local_tile_copy verify: OK")
    print(
